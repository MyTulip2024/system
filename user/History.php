    ...
    <script>
        ...
        function downloadHistory() {
            ...
            const tableData = filteredData.map(item => {
                ...
                const formattedGross = '₱' + item.gross_pay.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
                const formattedDeductions = '₱' + item.deductions.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
                const formattedNet = '₱' + item.net_pay.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
            ...
            // Format totals with proper peso sign
            const formattedTotalGross = '₱' + totalGross.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
            const formattedTotalDeductions = '₱' + totalDeductions.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
            const formattedTotalNet = '₱' + totalNet.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
