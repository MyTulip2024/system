...
function handleResendOTP($pdo) {
    ...
    try {
        ...
        $_SESSION['otp_success'] = 'New OTP has been generated successfully!';
...
// Get current OTP for display (since walang email/contact number)
$employee_number = $_SESSION['reset_employee_no'];
$stmt = $pdo->prepare("SELECT otp_code, expires_at FROM password_reset_otps WHERE employee_no = ? AND expires_at > NOW()");
$stmt->execute([$employee_number]);
$current_otp = $stmt->fetch(PDO::FETCH_ASSOC);
?>
...
<body>
    <div class="verify-container">
        <div class="verify-form">
            ...
            <!-- OTP Display (Since walang contact number/email) -->
            <?php if ($current_otp): ?>
                <div class="otp-display">
                    Your OTP Code: <strong><?php echo $current_otp['otp_code']; ?></strong>
                    <br><small>Expires: <?php echo date('h:i A', strtotime($current_otp['expires_at'])); ?></small>
                </div>
            <?php endif; ?>
