...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        try {
            ...
            $stmt = $conn->prepare("DELETE FROM attendance WHERE employee_id = :id");
            $stmt->bindParam(':id', $id);
            $stmt->execute();
            ...
            try {
                ...
                if ($hasDeletedColumn) {
                    ...
                    $stmt = $conn->prepare("UPDATE payroll_records SET deleted = 1 WHERE employee_id = :id");
                } else {
                    // Hard delete payroll records
                    $stmt = $conn->prepare("DELETE FROM payroll_records WHERE employee_id = :id");
                }
                $stmt->bindParam(':id', $id);
                $stmt->execute();
            } catch(PDOException $e) {
            ...
            try {
                $stmt = $conn->prepare("DELETE FROM thirteenth_month_pay WHERE employee_id = :id");
                $stmt->bindParam(':id', $id);
                $stmt->execute();
            } catch(PDOException $e) {
