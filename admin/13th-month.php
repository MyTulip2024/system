...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['release'])) {
        ...
        if (isset($_POST['employee_no'])) {
            ...
            $stmt = $conn->prepare("
                SELECT e.*, 
                       (SELECT SUM(days_worked) FROM payroll_records WHERE employee_id = e.id) as total_days_worked,
                       (SELECT SUM(net_salary) FROM payroll_records WHERE employee_id = e.id) as latest_net_salary
                FROM employees e 
                WHERE e.employee_no = ?
            ");
            ...
            $stmt = $conn->prepare("
                ...
                (employee_no, employee_id, year, months_worked, basic_pay, thirteenth_pay, status, created_at)
